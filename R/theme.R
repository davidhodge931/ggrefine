#' Wash theme
#'
#' @description A complete theme where the panel and plot share the same
#'   background colour. The default is a clean white background. Pass any
#'   colour to `panel_background_fill` to wash that colour across the whole
#'   plot area.
#'
#' @param panel_background_fill The fill of the panel.background and
#'   plot.background theme elements. Defaults to `"white"`.
...
#'   Defaults to `panel_background_fill`.
...
#' @param panel_widths The panel.widths theme element. A `unit` or unit vector setting
#'   the width of individual panels. Defaults to `NULL`.
#' @param panel_heights The panel.heights theme element. A `unit` or unit vector setting
#'   the height of individual panels. Defaults to `NULL`.
#'
...
#'   scale_colour_discrete(palette = paletteblend::multiply(scales::pal_hue()))
#'
#' p2 + theme_tint(flexoki::flexoki$blue["blue50"])
#' p2 + theme_wash(flexoki::flexoki$blue["blue50"])
#' p2 + theme_glow(flexoki::flexoki$blue["blue50"])
#' p2 + theme_black(flexoki::flexoki$blue["blue950"])
#'
theme_wash <- function(
    ...
    plot_background_fill = panel_background_fill,
    geom_fill = "#357BA2FF",
  ...
  theme <-
    ggplot2::theme(
      text = ggplot2::element_text(
        size = text_size, family = text_family, colour = text_colour,
        lineheight = 0.9, hjust = 0.5, vjust = 0.5, angle = 0,
        margin = ggplot2::margin()
      ),
      axis.line = ggplot2::element_line(
        colour = axis_line_colour, linewidth = axis_line_linewidth, lineend = "square"
      ),
      axis.line.x = NULL, axis.line.x.bottom = NULL, axis.line.x.top = NULL,
      axis.line.y = NULL, axis.line.y.left = NULL, axis.line.y.right = NULL,
      ...
      axis.ticks.length.x = NULL, axis.ticks.length.x.top = NULL, axis.ticks.length.x.bottom = NULL,
      axis.ticks.length.y = NULL, axis.ticks.length.y.left = NULL, axis.ticks.length.y.right = NULL,
      axis.minor.ticks.length = ggplot2::rel(0.66),
      axis.title = ggplot2::element_text(),
      axis.text  = ggplot2::element_text(),
      axis.text.y = ggplot2::element_text(margin = ggplot2::margin(r = 2.75), hjust = 1),
      axis.text.y.right = ggplot2::element_text(margin = ggplot2::margin(l = 2.75), hjust = 0),
      ...
      panel.grid.major = ggplot2::element_line(linewidth = panel_grid_linewidth, linetype = panel_grid_linetype),
      panel.grid.major.x = NULL, panel.grid.major.y = NULL,
      panel.grid.minor = ggplot2::element_line(linetype = panel_grid_minor_linetype, linewidth = panel_grid_minor_linewidth),
      ...
      strip.clip = "off",
      strip.text = ggplot2::element_text(margin = ggplot2::margin(t = 0, r = 0, b = 8, l = 0)),
      strip.text.x = NULL,
      strip.text.x.bottom = ggplot2::element_text(margin = ggplot2::margin(t = 8, r = 0, b = 0, l = 0)),
      strip.text.y = ggplot2::element_text(margin = ggplot2::margin(t = 0, r = 0, b = 0, l = 8), angle = -90),
      strip.text.y.left = ggplot2::element_text(margin = ggplot2::margin(t = 0, r = 8, b = 0, l = 0), angle = 90),
      ...
      plot.title = ggplot2::element_text(
        size = title_size, family = title_family, colour = title_colour,
        hjust = 0, margin = ggplot2::margin(b = 27.50)
      ),
      plot.title.position = "plot",
      plot.subtitle = ggplot2::element_text(
        size = subtitle_size, family = subtitle_family, colour = subtitle_colour,
        hjust = 0, margin = ggplot2::margin(b = 22)
      ),
      plot.caption = ggplot2::element_text(
        size = caption_size, family = caption_family, colour = caption_colour,
        hjust = caption_hjust, margin = ggplot2::margin(t = 7.5, r = 0, b = 0, l = 0)
      ),
    ...
    ggplot2::theme(
      geom = ggplot2::element_geom(
        fill = geom_fill, colour = geom_colour,
        pointshape = 21, linewidth = 0.66, borderwidth = 0.33, pointsize = 1.5
      ),
...
#'   The panel colour is visible against the clean white surround. Pass any
#'   colour to `panel_background_fill` to set the tint.
#'
#' @param panel_background_fill The fill of the panel.background theme element.
#'   Defaults to `flexoki::flexoki$base["base50"]`.
#' @inheritParams theme_wash
#'
#' @return A ggplot theme.
#' @export
#'
#' @inherit theme_wash examples
#'
theme_tint <- function(
  ...
  theme_wash(
    ...
    ...,
    text_size = text_size, text_family = text_family, text_colour = text_colour,
    legend_place = legend_place,
    legend_axis_line_colour = legend_axis_line_colour,
    legend_axis_line_linewidth = legend_axis_line_linewidth,
    legend_background_fill = legend_background_fill,
    ...
    legend_ticks_linewidth = legend_ticks_linewidth,
    legend_ticks_length = legend_ticks_length,
    axis_line_colour = axis_line_colour, axis_line_linewidth = axis_line_linewidth,
    axis_ticks_colour = axis_ticks_colour, axis_ticks_linewidth = axis_ticks_linewidth,
    axis_ticks_length = axis_ticks_length,
    panel_grid_colour = panel_grid_colour,
    panel_grid_linetype = panel_grid_linetype, panel_grid_linewidth = panel_grid_linewidth,
    ...
    geom_fill = geom_fill, geom_colour = geom_colour,
    ...
    panel_widths = panel_widths, panel_heights = panel_heights
  )
}

#' Glow theme
#'
#' @description A complete theme where the plot background is a darkened version
#'   of the panel colour, making the panel appear to glow against the deeper
#'   surround. Pass any colour to `panel_background_fill` to set the tint.
#'
#' @param panel_background_fill The fill of the panel.background theme element.
#'   Defaults to `flexoki::flexoki$base["base50"]`.
#' @inheritParams theme_wash
...
theme_glow <- function(
    ...
    panel_grid_minor_linetype = 1,
    panel_grid_minor_linewidth = 0.5,
    plot_background_fill = paletteblend::multiply(panel_background_fill),
    geom_fill = "#357BA2FF",
    geom_colour = geom_fill,
    palette_fill_discrete = jumble::jumble,
    palette_colour_discrete = palette_fill_discrete,
...
#' Black theme
#'
#' @description A complete theme with a dark panel on a black plot background.
#'   Text colours are light to contrast against the dark backgrounds.
#'
#' @inheritParams theme_wash
#'
#' @return A ggplot theme.
