# new_scale_fill_contrast ------------------------------------------------

#' Create a new contrasting colour scale based on fill
#'
#' @description
#' `new_scale_fill_contrast()` resets the active `colour` scale via
#' [ggnewscale::new_scale_colour()] and applies dynamic, high-contrast text or
#' outline colours based on the active layer's `fill` aesthetic using
#' [aes_fill_contrast()].
#'
#' Place `new_scale_fill_contrast()` before the target geom layer (e.g.,
#' `geom_text()`) so that elements inherit the updated contrasting colour
#' mapping.
#'
#' @param dark A dark colour string or hex value. If `NULL`, defaults to
#'   the primary dark text colour from the active theme.
#' @param light A light colour string or hex value. If `NULL`, defaults to
#'   the primary light text colour from the active theme.
#' @param ... Unused. Included to support trailing commas.
#'
#' @return A `list` of `ggplot2` scale and aesthetic mapping components
#'   suitable for adding to a plot with `+`.
#'
#' @export
#' @examples
#' library(ggplot2)
...
#'     vjust = 1.33
#'   )
new_scale_fill_contrast <- function(dark = NULL, light = NULL, ...) {
  list(
    ggnewscale::new_scale_colour(),
    aes_fill_contrast(dark = dark, light = light)
  )
}
...
#'
#' @description
#' `aes_fill_contrast()` generates an aesthetic mapping for `colour` that
...
#'
#' Can be passed directly into [ggplot2::aes()] or spliced into layer mappings
#' using [rlang::!!!].
#'
#' @param dark A dark colour string or hex value. If `NULL`, defaults to
...
# new_scale_panel_contrast -----------------------------------------------

#' Create a new contrasting colour scale based on panel background
#'
#' @description
#' `new_scale_panel_contrast()` resets the active `colour` scale via
#' [ggnewscale::new_scale_colour()] and applies dynamic, high-contrast text or
#' outline colours based on the active theme's panel background fill using
#' [aes_panel_contrast()].
#'
#' Place `new_scale_panel_contrast()` before the target geom layer (e.g.,
#' `geom_text()`) so that elements inherit the updated contrasting colour
#' mapping.
#'
#' @param dark A dark colour string or hex value. If `NULL`, defaults to
...
#'   geom_col() +
#'   new_scale_panel_contrast() +
#'   geom_text(aes(y = n + 2, label = n), vjust = 0)
new_scale_panel_contrast <- function(dark = NULL, light = NULL, ...) {
  list(
    ggnewscale::new_scale_colour(),
    aes_panel_contrast(dark = dark, light = light)
  )
}
...
#'
#' @description
#' `aes_panel_contrast()` generates an aesthetic mapping for `colour` that
#' automatically evaluates to a contrasting dark or light shade depending on
