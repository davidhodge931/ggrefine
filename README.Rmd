...
The themes are built to work with the refine functions in that they have all axis 
and panel grid elements.

They can also be customised easily. 

The `theme_greys` function has a smart `panel_grid_colour` default that is derived from the `panel_background_fill`. 
...
The package provides dynamic color and fill aesthetic and scale helpers that evaluate aesthetics late to provide automatic colour or fill properties. It features blend scales (scale_*_blend_*) to automatically compute complementary outline or fill, as well as contrast helpers (aes_fill_contrast(discrete = "none") and aes_panel_contrast(discrete = "none")) to adaptively select light or dark colours based on the fill or set panel background. 

It is recommended to use the ggrefine function, followed by ggplot2::aes, and change the colour/fill or palettes using update_palettes. This ensures these functions will work in all situations.
